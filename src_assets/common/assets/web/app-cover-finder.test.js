...
function candidatesFor(uuid) {
  return [
    { title: 'Hollow Knight', release_year: 2017, confidence: 0.98, token: TOKEN_ONE, preview: `./api/covers/preview/${TOKEN_ONE}?uuid=${uuid}`, expires_at: 1 },
    { title: 'Hollow Knight: Silksong', confidence: 0.61, token: TOKEN_TWO, preview: `./api/covers/preview/${TOKEN_TWO}?uuid=${uuid}`, expires_at: 1 },
  ]
}

function mountAppsView({ apps = [], search, select } = {}) {
...
describe('AppsView Find Cover', () => {
  ...
  it('puts the picked cover in the form and leaves saving to the player', async () => {
    ...
    await wrapper.findAll('[data-cover-candidate]')[1].trigger('click')
    await flushPromises()

    const [select] = coverCalls('select')
    expect(select[1].method).toBe('POST')
    expect(JSON.parse(select[1].body)).toEqual({ uuid: HOLLOW_UUID, token: TOKEN_TWO })
  ...
  it('gives a draft one temporary uuid for its search and its pick', async () => {
    ...
    await wrapper.findAll('[data-cover-candidate]')[0].trigger('click')
    await flushPromises()
  ...
  it('shows upstream trouble without a key link and keeps the tiles when a pick expires', async () => {
    const wrapper = mountAppsView({
      apps: [hollowKnight],
      select: () => reply(410, {
        status: false,
