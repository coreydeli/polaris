...
- The Spaces page changes every device at once. Device Access and Desktop Access gain **Select All** and **Clear All**, each a single change, where letting thirteen devices into a Space was thirteen saves and every save restarts Spaces. Clear All asks first, because the devices' Default Space there goes with them. `POST /api/multiseat/access/all` takes a Space or `desktop` and `allowed`, and the host picks the devices, so an out of date page cannot add one that was just unpaired.
- **Give Desktop Access with a Space**, a switch under Desktop Access. While it is on, a device you let into a Space can open Desktop too. It is off until you turn it on, since Desktop is your whole account, and it leaves devices you have already set up as they are.
- The Spaces page is shorter. A Space's card sums up who can open it, "Retroid Pocket 6, Pixel 10 Pro and 10 more", where it spelled out every name, and names its launcher. Default Space is one row per device instead of one card per device.
