...
- The Doctor names three Host Virtual Display problems on Plasma: another backend used in place of the KWin screen, with the reason; a stream screen at a scale other than 100%; and touch, pen or mouse that could not be tied to the stream screen, with KWin's answer. Troubleshooting lists every host finding now, not just the first, and a finding that needs nothing done reads as a note. The Virtual Display panel says why a KWin screen could not be used.
