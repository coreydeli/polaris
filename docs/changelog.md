...
- Unpairing a device also clears it from Spaces. Its id stayed in every Space's device list, in Desktop Access and as a Default Space for good; one test host still named a Default device that had been unpaired weeks earlier. The ids of devices that are no longer paired now leave those lists the next time a device's access is changed on the Spaces page, which is also when a device that was paired again needs its access back.
...
- Leaving a Space from the client is no longer logged as `Worker media failed: the worker transport ended`. The same clean exit was written down as an ended stream or as an error depending on which of two threads noticed the stop first, and with Steam Big Picture it was usually the error.
