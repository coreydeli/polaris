...
- A cover chosen in the app editor after artwork was picked for the entry in Nova now shows in Nova. The Nova pick outranked the entry's image, so the console showed the new cover while Nova kept the old poster. Saving the entry with another image, or with a cover Find Cover picked after the Nova pick, now drops the picked poster and keeps the picked background, logo and icon; saving any other change keeps the pick.
