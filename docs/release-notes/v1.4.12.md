...
- `sudo -H polaris --setup-host --enable-headless-boot` keeps Polaris running when the Deck switches between Desktop Mode and Game Mode.
...
- A second device can watch the running stream whatever resolution it would ask for. The host says up front what there is to watch, so a watcher is never refused for asking for the wrong mode.
