...
- A still screen on AMD no longer streams at the full bitrate in H.264 or HEVC. At constant bitrate the AMD driver pads frames up to the target with filler the client throws away, so an idle desktop at 20 Mbps sent 20 Mbps. Polaris now removes the filler before sending, and the same screen sends about 0.04 Mbps with the picture unchanged.
